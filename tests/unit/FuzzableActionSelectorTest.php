...
class FuzzableActionSelectorTest extends TestCase {
	...
	private array $addActionsMap = [
		"admin_menu" => [
			"testFunctionName",
			"testFunctionNameTwo",
			"testFunctionNameThree"
		],
		"init"       => [ "aivaha_booking_engine", "audio_record_engine" ]
	];
	...
	protected function setUp(): void {
		...
		$psalmErrorOne                       = new PsalmError( "TaintedHtml", "./fuzzable-action-selector/test.php" );
		$psalmErrorOne->errorMessage[]       = [ "id" => "\$_POST", "stmt" => "<no known location>" ];
		$psalmErrorOne->errorMessage[]       = [
			"id"   => "call to testFunctionName - ./fuzzable-action-selector/test.php:6:5",
			"stmt" => "return testFunctionName();"
		];
		$psalmPluginResultOne->psalmErrors[] = $psalmErrorOne;

		$psalmPluginResultTwo                = new PsalmPluginResult( "testSlugTwo", 2 );
		$psalmErrorTwo                       = new PsalmError( "TaintedHtml", "./fuzzable-action-selector/test.php" );
		$psalmErrorTwo->errorMessage[]       = [ "id" => "\$_POST", "stmt" => "<no known location>" ];
		$psalmErrorTwo->errorMessage[]       = [
			"id"   => "call to testFunctionNameTwo - ./fuzzable-action-selector/test.php:12:5",
			"stmt" => "return testFunctionNameTwo();"
		];
		$psalmErrorThree                     = new PsalmError( "TaintedHtml", "./fuzzable-action-selector/test.php" );
		$psalmErrorThree->errorMessage[]     = [ "id" => "\$_GET", "stmt" => "<no known location>" ];
		$psalmErrorThree->errorMessage[]     = [
			"id"   => "call to testFunctionNameThree - ./fuzzable-action-selector/test.php:25:5",
			"stmt" => "return testFunctionNameThree();"
		];
		...
		$this->fuzzableActionSelector = new FuzzableActionSelector( $this->addActionsMap, $psalmResults, "/fuzzable-action-selector" );
	}
